...
app.secret_key = 'inventory_simulation_secret_key'
...
def run_simulation():
    ...
    try:
        simulation_status = "Training RL model..."
...
def run_rl_training():
    try:
        ...
        # best model info
        if training_results.get('best_name'):
            result["best_model"] = training_results['best_name']
            result["best_cost"] = training_results['best_metrics']['total_cost']
        
        # individual agent costs
        if training_results.get('ppo_metrics'):
            result["ppo_cost"] = training_results['ppo_metrics']['total_cost']
...
def create_final_report(results, report):
    ...
    final_report.append("- Target is to Minimize overall cost")
    ...
    final_report.append("- **Product 2 Demand**: D = {5,4,3,2} with probabilities {1/8, 1/2, 1/4, 1/8}")
